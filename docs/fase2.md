...
1. **[capabilities.py](file:///f:/nispa-voiceover/backend/core/tts/capabilities.py)**:
   - Schema Pydantic `ModelCapabilities` con attributi tipizzati: `provider_id`, `model_id`, `display_name`, `supports_voice_clone`, `supports_voice_design`, `supports_batch`, `supports_native_dialogue`, `max_speakers`, `requires_reference_audio`, `requires_reference_transcript`, `supported_languages`, `sample_rate`, `execution`.
   - Eccezioni di dominio specifiche: `ModelNotFoundError`, `ProviderNotFoundError`, `VoiceNotFoundError`, `TranscriptRequiredError`, `OutOfMemoryError`, `SynthesisCancelledError`.
...
2. **[catalog.py](file:///f:/nispa-voiceover/backend/core/tts/catalog.py)**:
   ...
   - Mappa di alias retrocompatibile per garantire continuità sia con i nomi completi delle directory su disco (`Qwen3-TTS-12Hz-0.6B-Base`, `VibeVoice-1.5B`) sia con i riferimenti abbreviati (`Qwen3-TTS-1.7B`, `VibeVoice`).
   - `resolve_model_capabilities(model_id)` solleva `ModelNotFoundError` con suggerimento dei modelli validi se il modello non esiste.
...
- **[base.py](file:///f:/nispa-voiceover/backend/core/tts/base.py)**:
   - Aggiunto `unload()` al contratto `TTSProvider`.
   - Aggiunto fallback batch sequenziale comune in `synthesize_batch()`.
   - Aggiunto stub opzionale `synthesize_dialogue()`.
- **[qwen_provider.py](file:///f:/nispa-voiceover/backend/core/tts/qwen_provider.py)** & **[vibe_provider.py](file:///f:/nispa-voiceover/backend/core/tts/vibe_provider.py)**:
   - Implementato `unload()` pubblico che sposta i tensori su CPU, azzera i riferimenti e rilascia la cache CUDA.
- **[tts_provider.py](file:///f:/nispa-voiceover/backend/core/tts_provider.py)**:
   - `MultiModelProvider` ora delega a `ProviderRegistry` e `ModelCatalog`.
   - Eliminata la condizione `if "Qwen" in model_name`.
   - Preservate le proprietà `vibe`, `qwen` e i pool `_vibe_pool`, `_qwen_pool` per piena backward compatibility con test e router esistenti.

### 3. Risultati della Suite di Test
- **Test Unitari Provider (`backend/tests/test_provider.py`)**: **19 passed** (+9 nuovi test specifici per catalog, registry, capabilities e no-fallback).
- **Test Qwen Provider (`backend/tests/test_qwen_provider.py`)**: **3 passed**.
- **Backend totale (`run_tests.py --backend`)**: **106 passed** (0 errori, precedentemente 97).
- **Frontend totale (`run_tests.py --frontend`)**: **28 passed** (0 errori).

Tutti i task della **Fase 2** sono completati e verificati in [TASKS.md](file:///f:/nispa-voiceover/TASKS.md#L56-L95).
