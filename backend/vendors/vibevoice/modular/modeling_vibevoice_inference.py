...
class VibeVoiceForConditionalGenerationInference(VibeVoicePreTrainedModel, GenerationMixin):
    ...
    def _build_generate_config_model_kwargs(self, generation_config, inputs, tokenizer, return_processors=False, **kwargs):
        ...
        # Signature: (self, generation_config, model_kwargs, generation_mode, batch_size, max_cache_length)
        self._prepare_cache_for_generation(
            generation_config=generation_config,
            model_kwargs=model_kwargs,
            generation_mode="sample" if generation_config.do_sample else "greedy",
       ...
       # Define all valid tokens that can be generated
        ...
        for step in progress_bar:
            ...
            if diffusion_start_indices.numel() > 0 and kwargs.get('refresh_negative', True):
                ...
                # Some cache versions use key_cache/value_cache, others might use internal lists
                key_cache = getattr(negative_model_kwargs['past_key_values'], 'key_cache', [])
                value_cache = getattr(negative_model_kwargs['past_key_values'], 'value_cache', [])
                
                for layer_idx, (k_cache, v_cache) in enumerate(zip(key_cache, value_cache)):
